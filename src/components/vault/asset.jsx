...
class Asset extends Component {
    ...
    return (<div className={ classes.actionsContainer }>
      <div className={ classes.tradeContainer }>
        <div className={ classes.balances }>
            <Typography variant='h4' onClick={ () => { this.setAmount(100) } } className={ classes.value } noWrap>{ '余额: '+ (asset.balance ? (Math.floor(asset.balance*10000)/10000).toFixed(4) : '0.0000') } { asset.tokenSymbol ? asset.tokenSymbol : asset.symbol }</Typography>
        </div>
        <TextField
          fullWidth
        ...
        <div className={ classes.buttons }>
          { asset.deposit === true &&
            <Button
              ...
              onClick={ this.onMyDeposit }
              fullWidth
              >
              <Typography className={ classes.buttonText } variant={ 'h5'} color={asset.disabled?'':'secondary'}>存入</Typography>
            </Button>
          }
          { asset.depositAll === true &&
            <Button
              ...
              fullWidth
              >
              <Typography className={ classes.buttonText } variant={ 'h5'} color={asset.disabled?'':'secondary'}>全部存入</Typography>
            </Button>
          }
        </div>
        { asset.depositDisabled === true &&
          <div className={classes.disabledContainer}>
            <Typography variant='h4'>当前该Vault无法存款。</Typography>
          </div>
        }
      </div>
        ...
        <div className={ classes.buttons }>
          { asset.withdraw === true &&
            <Button
              ...
              fullWidth
              >
              <Typography className={ classes.buttonText } variant={ 'h5'} color='secondary'>取出</Typography>
            </Button>
          }
          { asset.withdrawAll === true &&
            <Button
              ...
              fullWidth
              >
              <Typography className={ classes.buttonText } variant={ 'h5'} color='secondary'>全部取出</Typography>
            </Button>
          }
        </div>
      </div>
  ...
  onMyDeposit = () => {
    console.log("onMyDeposit123")
    const { amount } = this.state
    const { asset, startLoading } = this.props
    dispatcher.dispatch({ type: TEST, content: { amount: amount, asset: asset } })
  }
  ...
  onDeposit = () => {
    ...
    if(!amount || isNaN(amount) || amount < 0 ) {//|| amount > asset.balance
      ...
      console.log("onDeposit() if")

      this.setState({ amountError: true })
      return false
    }

    console.log("onDeposit()  11")
