...
class Store {
  ...
  _checkApproval = async (asset, account, amount, contract, callback) => {
    ...
    if(asset.erc20address === 'Ethereum') {
      console.log("_checkApproval   callback()")
      return callback()
    }
    console.log("_checkApproval")
  ...
  getTest = async (payload) => {
    const account = store.getStore('account')
    console.log("_getTest   account==",account)
    const { asset, amount } = payload.content
    console.log("==getTest  asset ==",asset)
    console.log("==getTest  amount==",amount)
    const mm = ''+amount*10**18


    console.log("abi['golffABI']=====",config.golffABI)
    const web3 = new Web3(store.getStore('web3context').library.provider);
    let myContract = new web3.eth.Contract(config.golffABI, '0xC46188A5a7c1dFB04a99903544660EA83f986FB8');//0xf3D12f4e4C42c530eeb010520b7325789C64C6a7   0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2
    console.log("myContract=-=-=-=-=-=-",myContract);
    myContract.methods.stake(mm).send({ from: account.address ,gas: 200000 }).then(result => {
            console.log("cg")
          }).catch(err => {
            console.log("sb")
          });
    // myContract.methods.deposit().send({ from: account.address, value: 0.3*10**18, gasPrice: web3.utils.toWei(await this._getGasPrice(), 'gwei') })
    //     .on('transactionHash', function(hash){
    //       console.log("111")
    //       console.log(hash)
    //     })
  ...
  getBalances = async () => {
    ...
    async.map(assets, (asset, callback) => {
      async.parallel([
        ...
        asset.investedBalance = data[2]
        asset.price = data[3]
        asset.maxApr = data[4]
        asset.poolValue = data[5]
  ...
  _getERC20Balance = async (web3, asset, account, callback) => {
    //获取余额
    if(asset.erc20address === 'Ethereum') {
      console.log("erc20address==Ethereum")
      try {
  ...
  _getBalance = async (web3, asset, account, callback) => {
    ...
    console.log("===================_getBalance001")
    if(asset.erc20address === 'Ethereum') {
      try {
        console.log("===================_getBalance")
        const eth_balance = web3.utils.fromWei(await web3.eth.getBalance('0x962319512CDA375F4b5BeC405a814Ff3982cBb07'), "ether");//asset.iEarnContract
        callback(null, parseFloat(eth_balance))
      } catch(ex) {
        console.log(ex)
        return callback(ex)
  ...
  depositVault = (payload) => {
    ...
    const account = store.getStore('account')
    console.log("==depositVault==",account)
    const { asset, amount } = payload.content
    console.log("==depositVault  asset ==",asset)
    console.log("==depositVault  amount==",amount)
  ...
  _callDepositVault = async (asset, account, amount, callback) => {
    ...
    console.log("调用 _callDepositVault")
    const web3 = new Web3(store.getStore('web3context').library.provider);

    //**********

    let vaultContract = new web3.eth.Contract(asset.vaultContractABI, asset.vaultContractAddress)   //**********
    console.log("vaultContractAddress==",asset.vaultContractAddress)
    ...
    if(asset.erc20address === 'Ethereum') {
      console.log("asset.erc20address === 'Ethereum'")
